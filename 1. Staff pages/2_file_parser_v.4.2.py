...
# у версії 3.0 було оновлено regex , який виділяє вміст розділу із загальною інформацією про викладача
# у попередній версії: r"""<div class='info'>(.*?)</div>"""
# у поточній версії:   r"""<div class='info'>(.*?)</div></section>"""
# тег </div> може міститися і всередині даного розділу
# (така ситуація була виявлена в html-коді сторінок викладачів фізичного факультету),
# тимчасом як комбінація </div></section> міститься лише наприкінці розділу

# у версії 4.0 рядки
# if 'Біографія' in self.sections:
#     self.biography = re.search(r"""<section class='section'><h2>Біографія</h2>(.*?)</section>""", html_main).groups()[0]
# було замінено на
# if {'Біографія', 'Наукова біографія'} & self.sections:
#     self.biography = re.search(r"""<section class='section'><h2>(Наукова )?[Бб]іографія</h2>(.*?)</section>""", html_main).groups()[1]

# у версії 4.1 файли зберігаються під іменами, які містять інформацію про факультет
# Деякі викладачі працюють і на факультеті, і в коледжі.
# Тому необхідно проаналізувати дані з обох сторінок - і на сайті ф-ту, і на сайті коледжу
#
# У версії 4.2 програма генерує ще один файл - зі списком курсів
...
class Lecturer:
    ...
    def __init__(self, link, html_main=""):
        ...
        try:
            ...
            self.general_info = General_info(
                re.search(r"""<div class='info'>(.*?)</div></section>""", html_main).groups()[0]
            )
            ### REGEX UPDATED!!! 02.04.2018. Old one: r"""<div class='info'>(.*?)</div>"""
            ### But </div> can be encoumtered earlier in general info section!
            ### так було на сторінках викладачів фізичного факультету
...
def _main():
    ...
    courses = set()  # # # NEW
    # just compare: list - 5345, set - 3409
    ...
    for elem in links:
        ...
        with open(filename, 'rt', encoding="utf-8") as f:
            ...
            courses.update(set(lecturer.courses.href_list))  # # # NEW
    ...
    filename = "all_courses.txt"  # # # NEW
    with open(filename, "wt", encoding="utf-8") as f:
        for course_link in sorted(courses):
            f.write(course_link + "\n")
